...
public class DHDFuelScreen extends SGScreen {
    ...
    protected void drawBackgroundLayer() {
        bindTexture(SGCraft.getInstance().resourceLocation("textures/gui/dhd_fuel_gui.png"), 256, 256);
        ...
        int naquadahUnits = te.getInventory().getStackInSlot(0).getCount() + te.getInventory().getStackInSlot(1).getCount() + te.getInventory().getStackInSlot(2).getCount() + te.getInventory().getStackInSlot(3).getCount();

        // Buffer Available
        drawRightAlignedString("DHD Buffer:", 125, 30);
        drawString(dFormat.format(Math.min(Math.max(te.energyInBuffer, 0), DHDTE.maxEnergyBuffer)), 130, 30);

        // Buffer Max
        drawRightAlignedString("Buffer Max:", 125, 40);
        drawString(dFormat.format(DHDTE.maxEnergyBuffer), 130, 40);
        ...
        drawString(dFormat.format(naquadahUnits * SGConfiguration.energyPerFuelItem), 130, 70);
